(function () {
	...
	frappe.views.ListView = class ListView extends frappe.views.BaseList {
		...
		get_column_html(col, doc) {
			...
			var translated_doctypes = (frappe.boot && frappe.boot.translated_doctypes) || [];
		...
		setup_realtime_updates() {
			...
			frappe.realtime.on("list_update", function (data) {
				if (!data || (data.doctype !== this$1.doctype)) {
					return;
				}
				if (data.doctype && data.name) {
					var doc = frappe.get_doc(data.doctype, data.name);
				...
				this$1.pending_document_refreshes.push(data);
				frappe.utils.debounce(this$1.process_document_refreshes.bind(this$1), 1000)();
			});
		}
		...
		process_document_refreshes() {
			...
			var names = this.pending_document_refreshes.map(function (d) { return d.name; });
			this.pending_document_refreshes = this.pending_document_refreshes.filter(
				function (d) { return names.indexOf(d.name) === -1; }
			);

			if (!names.length) { return; }

			// filters to get only the doc with this name
			var call_args = this.get_call_args();
			...
			frappe.call(call_args).then(function (ref) {
				...
				if (!(data && data.length)) {
					...
					this$1.data = this$1.data.filter(function (d) { return names.indexOf(d.name) === -1; });
					this$1.render_list();
					return;
				}

				data.forEach(function (datum) {
					var index = this$1.data.findIndex(function (doc) { return doc.name === datum.name; });
	...
	frappe.views.ListViewSelect = class ListViewSelect {
		...
		get_email_accounts() {
			...
			accounts.forEach(function (account) {
				...
				var route = "/app/communication/view/inbox/" + email_account;
				var display_name = [
					"All Accounts",
					"Sent Mail",
					"Spam",
