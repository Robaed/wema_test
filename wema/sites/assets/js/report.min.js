var Rollup = (function (exports) {
	...
	class RowManager {
	    ...
	    getFilterInput(props) {
	        var title = "title=\"Filter based on " + (props.name || 'Index') + "\"";
	        var dataAttr = makeDataAttributeString(props);
	...
	frappe.views.ReportView = class ReportView extends frappe.views.ListView {
		...
		setup_events() {
			...
			frappe.realtime.on("list_update", function (data) { return this$1.on_update(data); });
		}
		...
		render_editing_input(colIndex, value, parent) {
			...
			if (col.docfield.fieldtype === 'Text Editor') {
				var d = new frappe.ui.Dialog({
					...
					on_hide: function () {
						this$1.datatable.cellmanager.deactivateEditing(false);
					}
				});
				d.show();
		...
		add_currency_column(fieldname, doctype, col_index) {
			...
			if (df && df.fieldtype === 'Currency' && df.options &&
				...
				var field_label = frappe.meta.get_label(doctype, field[0]);
				frappe.show_alert(
		...
		add_status_dependency_column(col, doctype) {
			...
			if (col && !this.fields.find(function (f) { return f[0] === col; })) {
			...
			return this.columns.map(function (col) {
				...
				if (col.field === 'docstatus' && !frappe.meta.has_field(this$1.doctype, 'status')) {
					...
					if (status) {
						...
						var dependent_col = status[2] && status[2].split(',')[0];
		...
		report_menu_items() {
			...
			var can_edit_or_delete = function (action) {
				var method = action == "delete" ? "can_delete" : "can_write";
				return (
					this$1.report_doc
					&& this$1.report_doc.is_standard !== "Yes"
					&& (
						frappe.model[method]("Report")
						|| this$1.report_doc.owner === frappe.session.user
					)
				);
			};
			...
			if (can_edit_or_delete()) {
				items.push({
					label: __("Save"),
					action: function () { return this$1.save_report('save'); }
				});
			}
			...
			items.push({
				label: __('Save As'),
				action: function () { return this$1.save_report('save_as'); }
			});
			...
			if (can_edit_or_delete("delete")) {
				items.push({
					label: __("Delete"),
					action: function () { return frappe.confirm(
						"Are you sure you want to delete this report?",
						function () { return this$1.delete_report(); }
					); },
	...
	frappe.views.QueryReport = class QueryReport extends frappe.views.BaseList {
		...
		load_report(route_options) {
			...
			frappe.run_serially([
				...
				function () { return this$1.refresh_report(route_options); },
		...
		refresh_report(route_options) {
			...
			return frappe.run_serially([
				...
				function () { return this$1.set_route_filters(route_options); },
		...
		set_route_filters(route_options) {
			...
			if (route_options) {
				var fields = Object.keys(route_options);
				...
				var promises = filters_to_set.map(function (f) {
					return function () {
						var value = route_options[f.df.fieldname];
						f.set_value(value);
					};
				});
			...
			return new Promise(function (resolve) {
				this$1.last_ajax = frappe.call({
					...
					args: {
						report_name: this$1.report_name,
						filters: filters,
						is_tree: this$1.report_settings.tree,
						parent_field: this$1.report_settings.parent_field
					},
					callback: resolve,
				...
				if (data.prepared_report) {
					...
					this$1.prepared_report_document = data.doc;
		...
		pdf_report(print_settings) {
			...
			var filter_values = [],
				name_len = 0;
			for (var i = 0, list = Object.keys(applied_filters); i < list.length; i += 1) {
				var key = list[i];
		...
		toggle_nothing_to_show(flag) {
			var message = (this.prepared_report && !this.prepared_report_document)
	...
	frappe.templates['print_grid'] = ' {% if title %} <h2>{{ __(title) }}</h2> <hr> {% endif %} {% if subtitle %} {{ subtitle }} <hr> {% endif %} <table class="table table-bordered">    <thead>   <tr>   <th> # </th>   {% for col in columns %}    {% if col.name && col._id !== "_check" %}    <th     {% if col.minWidth %}      style="min-width: {{ col.minWidth }}px"     {% endif %}     {% if col.docfield && frappe.model.is_numeric_field(col.docfield) %}      class="text-right"     {% endif %}    >     {{ __(col.name) }}</th>    {% endif %}   {% endfor %}   </tr>  </thead>    <tbody>   {% for row in data %}    <tr style="height: 30px">    <td {% if row.bold == 1 %} style="font-weight: bold" {% endif %}>     <span> {{ row._index + 1 }} </span>    </td>    {% for col in columns %}     {% if col.name && col._id !== "_check" %}      {% var value = col.fieldname ? row[col.fieldname] : row[col.id] %}      {% var longest_word = cstr(value).split(" ").reduce((longest, word) => word.length > longest.length ? word : longest, ""); %}      <td {% if row.bold == 1 %} style="font-weight: bold" {% endif %} {% if longest_word.length > 45 %} class="overflow-wrap-anywhere" {% endif %}>       <span {% if col._index == 0 %} style="padding-left: {%= cint(row.indent) * 2 %}em" {% endif %}>        {% format_data = row.is_total_row && ["Currency", "Float"].includes(col.fieldtype) ? data[0] : row %}        {% if (row.is_total_row && col._index == 0) { %}         {{ __("Total") }}        {% } else { %}         {{          col.formatter           ? col.formatter(row._index, col._index, value, col, format_data, true)           : col.format            ? col.format(value, row, col, format_data)            : col.docfield             ? frappe.format(value, col.docfield)             : value         }}        {% } %}       </span>      </td>     {% endif %}    {% endfor %}    </tr>   {% endfor %}  </tbody> </table> ';

	frappe.templates['print_tree'] = '<!DOCTYPE html> <html lang="en">  <head>   <meta charset="utf-8">   <meta http-equiv="X-UA-Compatible" content="IE=edge">   <meta name="viewport" content="width=device-width, initial-scale=1">   <meta name="description" content="">   <meta name="author" content="">   <title>{{ title }}</title>   <link href="{{ base_url }}/assets/frappe/css/bootstrap.css" rel="stylesheet">   <link type="text/css" rel="stylesheet"    href="{{ base_url }}/assets/frappe/css/font-awesome.css">   <link rel="stylesheet" type="text/css" href="{{ base_url }}/assets/frappe/css/tree.css">   <link rel="stylesheet" type="text/css" href="{{ base_url }}{{ print_format_css_path }}">   <style>    {{ print_css }}   </style>   <style>    .tree.opened::before,    .tree-node.opened::before,    .tree:last-child::after,    .tree-node:last-child::after {     z-index: 1;     border-left: 1px solid #d1d8dd;     background: none;    }    .tree a,    .tree-link {     text-decoration: none;     cursor: default;    }    .tree.opened > .tree-children > .tree-node > .tree-link::before,    .tree-node.opened > .tree-children > .tree-node > .tree-link::before {     border-top: 1px solid #d1d8dd;     z-index: 1;     background: none;    }    i.fa.fa-fw.fa-folder {     z-index: 2;     position: relative;    }    .tree:last-child::after, .tree-node:last-child::after {     display: none;    }    .tree-node-toolbar {     display: none;    }    i.octicon.octicon-primitive-dot.text-extra-muted {     width: 7px;     height: 7px;     border-radius: 50%;     background: #d1d8dd;     display: inline-block;     position: relative;     z-index: 2;    }     @media (max-width: 767px) {     ul.tree-children {      padding-left: 20px;     }    }   </style>    </head>  <body>   <svg id="frappe-symbols" aria-hidden="true" style="position: absolute; width: 0; height: 0; overflow: hidden;" class="d-block" xmlns="http://www.w3.org/2000/svg">    <symbol viewBox="0 0 12 12" xmlns="http://www.w3.org/2000/svg" id="icon-primitive-dot">     <path d="M9.5 6a3.5 3.5 0 1 1-7 0 3.5 3.5 0 0 1 7 0z"></path>    </symbol>     <symbol viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" id="icon-folder-open">     <path d="M8.024 6.5H3a.5.5 0 0 0-.5.5v8a2 2 0 0 0 2 2h11a2 2 0 0 0 2-2V9.5A.5.5 0 0 0 17 9h-6.783a.5.5 0 0 1-.417-.224L8.441 6.724a.5.5 0 0 0-.417-.224z" stroke="var(--icon-stroke)" stroke-miterlimit="10" stroke-linecap="square"></path>     <path d="M3.88 4.5v-1a.5.5 0 0 1 .5-.5h11.24a.5.5 0 0 1 .5.5V7" stroke="var(--icon-stroke)" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"></path>    </symbol>     <symbol viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" id="icon-folder-normal">     <path d="M2.5 4v10a2 2 0 0 0 2 2h11a2 2 0 0 0 2-2V6.5a1 1 0 0 0-1-1h-6.283a.5.5 0 0 1-.417-.224L8.441 3.224A.5.5 0 0 0 8.024 3H3.5a1 1 0 0 0-1 1z" stroke="var(--icon-stroke)" stroke-miterlimit="10" stroke-linecap="square"></path>    </symbol>   </svg>   <div class="print-format-gutter">    {% if print_settings.repeat_header_footer %}     <div id="footer-html" class="visible-pdf">      {% if print_settings.letter_head && print_settings.letter_head.footer %}       <div class="letter-head-footer">        {{ print_settings.letter_head.footer }}       </div>      {% endif %}      <p class="text-center small page-number visible-pdf">       {{ __("Page {0} of {1}", [`<span class="page"></span>`, `<span class="topage"></span>`]) }}      </p>     </div>    {% endif %}     <div class="print-format {% if landscape %} landscape {% endif %}">     {% if print_settings.letter_head %}     <div {% if print_settings.repeat_header_footer %} id="header-html" class="hidden-pdf" {% endif %}>      <div class="letter-head">{{ print_settings.letter_head.header }}</div>     </div>      {% endif %}      <div class="tree opened">       {{ tree }}      </div>    </div>   </div>  </body> </html> ';

	frappe.templates['group_by'] = '<div class="group-by-box">  <div class="visible-xs flex justify-flex-end">   <span class="remove-group-by">    {{ __("Remove") }}   </span>  </div>  <div class="list_groupby row">   <div class="col-sm-8 form-group">    <select class="group-by form-control input-xs">     <option value="" disabled selected>{{ __("Select Group By...") }}</option>     {% for (var parent_doctype in group_by_conditions) { %}      {% for (var val in group_by_conditions[parent_doctype]) { %}       {% if (parent_doctype !== doctype) { %}       <option        data-doctype="{{parent_doctype}}"        value="{{group_by_conditions[parent_doctype][val].fieldname}}"       >        {{ __(group_by_conditions[parent_doctype][val].label) }}        ({{ __(parent_doctype) }})       </option>       {% } else { %}       <option        data-doctype="{{parent_doctype}}"        value="{{group_by_conditions[parent_doctype][val].fieldname}}"       >        {{ __(group_by_conditions[parent_doctype][val].label) }}       </option>       {% } %}      {% } %}     {% } %}    </select>   </div>   <div class="col-sm-3 form-group">    <select class="aggregate-function form-control input-xs">     {% for condition in aggregate_function_conditions %}     <option value="{{condition.name}}">{{ __(condition.label) }}</option>     {% endfor %}    </select>   </div>   <div class="col-sm-4 col-xs-12" style="display: none">    <select class="aggregate-on form-control input-xs">     <option value="" disabled selected>{{ __("Select Field...") }}</option>    </select>   </div>   <div class="groupby-actions pull-left col-sm-1 hidden-xs">    <span class="remove-group-by">     <svg class="icon icon-sm">      <use href="#icon-close"></use>     </svg>    </span>   </div>  </div> </div> ';
